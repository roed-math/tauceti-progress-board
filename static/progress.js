...
// snapshot, like the chart SVGs). No dependencies; every string from the data is escaped.
(function () {
  "use strict";
  ...
  var REASON_TEXT = {
    "no-layers": "the README has no layer headings, so there is nothing to assess",
    ...
    "transcription-retired": "the report or the README changed after the layer states were transcribed, so the transcription was retired",
  ...
  function validate(data) {
    if (!data || data.schema_version !== 2) return "unexpected data version";
    if (!Array.isArray(data.rows) || !Array.isArray(data.weeks) || !data.global || !Array.isArray(data.global.weekly)) return "missing rows, weeks or global figures";
    if (typeof data.generated_at !== "string" || isNaN(Date.parse(data.generated_at))) return "missing generation time";
    for (var i = 0; i < data.rows.length; i++) {
      var r = data.rows[i];
      if (typeof r.name !== "string" || !Array.isArray(r.layers) || !Array.isArray(r.states) || r.layers.length !== r.states.length) return "malformed row " + i;
      for (var j = 0; j < r.states.length; j++) if (STATES.indexOf(r.states[j]) < 0) return "unknown layer state in " + r.name;
      if (r.activity && (!Array.isArray(r.activity.weekly) || r.activity.weekly.length !== data.weeks.length)) return "malformed activity in " + r.name;
      if (!r.assessment) return "missing assessment in " + r.name;
    }
    return null;
  }
  ...
  function render(data) {
    var rows = data.rows;
    var generated = data.generated_at;
    var generatedDay = generated.slice(0, 10);
    ...
    function isLeaf(r) { return !kids(r.name).length; }
    ...
    function reportAge(r) { return r.status && r.status.ts ? daysBetween(r.status.ts, generated) : null; }
    function lastAge(r) { return r.activity && r.activity.last ? daysBetween(r.activity.last, generated) : null; }
    function agoText(n) {
      if (n === null) return "";
      if (n <= 0) return "on the data date";
    ...
    function bars(values, w, h, max, label) {
      ...
      values.forEach(function (v, i) {
        ...
        s += '<rect x="' + x.toFixed(2) + '" y="' + (h - 1 - bh).toFixed(2) + '" width="' + bw.toFixed(2) + '" height="' + bh.toFixed(2) + '" rx="1" class="' + (now ? "pb-bar-now" : "pb-bar") + '"><title>week of ' + data.weeks[i] + ": " + v + " merged" + (now ? " (current week, incomplete)" : "") + "</title></rect>";
      });
      return s + "</svg>";
    }
    function weeklyTable(values) {
      var s = '<table class="pb-weeks"><caption>Merged pull requests per week, weeks starting Monday; the last week is incomplete</caption><tbody>';
      values.forEach(function (v, i) { s += "<tr><th scope=\"row\">" + data.weeks[i] + "</th><td>" + v + "</td></tr>"; });
    ...
    function legend() {
      return '<div class="pb-legend pb-legend-main" aria-label="Layer states">' +
        ...
        '<span class="pb-legend-note">States are what each roadmap’s latest report says, not a percentage of the work.</span></div>';
    }
    ...
    function tiles() {
      ...
      var un = g.unattributed || {};
      var unN = (un.no_label || 0) + (un.several_labels || 0) + (un.unknown_area || 0);
      ...
      return '<div class="pb-tiles">' +
        ...
        '<div class="pb-tile"><div class="pb-label">Layers, all roadmaps</div><div class="pb-big">' + tot.done + "<small>of " + layerTotal + " reported done</small></div>" +
          stack(tot) +
          '<div class="pb-legend">' + STATES.map(function (s) { return '<span><span class="pb-sw ' + STATE_CLASS[s] + '"></span><b>' + tot[s] + "</b> " + (s === "unassessed" ? "unassessed" : s) + "</span>"; }).join("") + "</div></div>" +
        '<div class="pb-tile"><div class="pb-label">Merged pull requests</div><div class="pb-big">' + g.last30 + "<small>in 30 days, all of Tau Ceti</small></div>" +
          bars(g.weekly, 200, 34, gmax, "All merged pull requests per week, " + data.weeks.length + " weeks") +
          '<div class="pb-sub"><b>' + g.total + "</b> merged" + (g.first_merge ? " since " + esc(g.first_merge) : "") + (unN ? " · <b>" + unN + "</b> with no single roadmap label" : "") + "</div></div>" +
        '<div class="pb-tile"><div class="pb-label">Reports</div><div class="pb-big">' + reported.length + "<small>of " + tops.length + " roadmaps</small></div>" +
          '<div class="pb-sub">' + (ages.length ? "oldest <b>" + ages[ages.length - 1] + "</b> days before the data date · " : "") + "<b>" + dueN + "</b> due an update (" + data.update_due_prs + "+ PRs since)</div></div>" +
        "</div>";
    }
    ...
    function passesShow(r) {
      switch (state.show) {
        ...
        case "unassessed": return r.states.indexOf("unassessed") >= 0 || (aggregate(r) ? aggregate(r).c.unassessed > 0 : false);
    ...
    function covText(r) {
      ...
      if (!n) return '<div class="pb-covtext"><span class="pb-none">no layers</span></div>';
      var a = r.assessment;
      if (!ag && a.reason !== "ok") return '<div class="pb-covtext"><span class="pb-none">' + esc({ "no-report": "no report yet", "not-transcribed": "report not transcribed", "transcription-retired": "transcription retired", "invalid-marker": "coverage marker invalid" }[a.reason] || a.reason) + '</span><br><span class="pb-faint">' + plural(n, "layer") + " unassessed</span></div>";
      var parts = [];
    ...
    function activityCell(r) {
      if (!r.activity) return '<div class="pb-covtext"><span class="pb-none">' + (r.parent ? "shares the parent’s label" : "no labelled pull requests") + "</span></div>";
    ...
    function lastCell(r) {
      if (!r.activity || !r.activity.last) return '<div class="pb-num"><span class="pb-sub">' + (r.parent ? "" : "—") + "</span></div>";
      return '<div class="pb-num">' + esc(r.activity.last) + '<span class="pb-sub">' + esc(agoText(lastAge(r))) + " · " + r.activity.total + " merged</span></div>";
    }
    function reportCell(r) {
      ...
      return '<div class="pb-age' + (due(r) ? " stale" : "") + '">' + esc((s.ts || "").slice(0, 10)) + '<span class="pb-sub">library <span class="pb-sha">' + esc(s.to_sha.slice(0, 7)) + "</span></span>" +
    ...
    function rowHtml(r, ctx) {
      var open = state.open.indexOf(r.name) >= 0;
      var ch = kids(r.name);
      var kidsOpen = state.kids.indexOf(r.name) >= 0 || (state.q && ch.some(function (k) { return matchInfo(k).hit; }));
      var mi = matchInfo(r);
      var name = '<div class="pb-namecell">' +
        '<button type="button" class="pb-toggle" aria-expanded="' + open + '" aria-controls="rd-' + esc(r.name) + '" data-open="' + esc(r.name) + '" title="' + (open ? "Hide" : "Show") + ' the report for ' + esc(r.name) + '"><span class="pb-chev" aria-hidden="true">' + (open ? "▾" : "▸") + '</span><span class="pb-vh">' + (open ? "Hide" : "Show") + " details for " + esc(r.name) + "</span></button>" +
        '<div><div class="pb-name"><a href="' + ROADMAP_REPO + "/blob/main/" + esc(r.readme) + '">' + esc(r.name) + "</a>" + chips(r) + (ctx ? '<span class="pb-chip">context</span>' : "") + "</div>" +
        '<div class="pb-title">' + esc(r.title) + "</div>" +
        (mi.where ? '<div class="pb-matched">matches ' + esc(mi.where) + "</div>" : "") +
        (ch.length ? '<button type="button" class="pb-kids" aria-expanded="' + kidsOpen + '" data-kids="' + esc(r.name) + '">' + (kidsOpen ? "hide " : "show ") + plural(ch.length, "sub-roadmap") + "</button>" : "") +
        "</div></div>";
      return '<tr class="pb-row' + (r.parent ? " child" : "") + '" id="rm-' + esc(r.name) + '">' +
        ...
        '<tr class="pb-detail" id="rd-' + esc(r.name) + '"' + (open ? "" : " hidden") + '><td colspan="6">' + (open ? detailHtml(r) : "") + "</td></tr>";
    }
    function pinned(path) { return ROADMAP_REPO + "/blob/" + (data.roadmap_head || "main") + "/" + path; }
    function detailHtml(r) {
      ...
      if (!r.parent) sources.push('<a href="' + GH_PRS + esc(r.name) + '">merged pull requests with this label</a>');
      if (r.completed) sources.push('<a href="' + ROADMAP_REPO + '/blob/main/Completed/README.md">completion decision</a>');
      left += '<div class="pb-links">' + sources.join("") + '<button type="button" class="pb-copy" data-copy="' + esc(r.name) + '">Copy link to this roadmap</button></div>';
      var evid = "Topic: " + esc(r.topic) + " (a hand assignment). ";
      if (a.reason === "ok") evid += "Layer states come from " + (a.source === "marker" ? "the coverage marker in the report" : "a hand transcription of the report’s prose, bound to that exact report") + "; they are the report’s assessment at library commit " + esc(s.to_sha.slice(0, 7)) + ", not a certificate that each layer’s specification is fully met. ";
      else evid += "Layer states are unassessed: " + esc(REASON_TEXT[a.reason] || a.reason) + (a.detail ? " (" + esc(a.detail) + ")" : "") + ". ";
      ...
      if (r.layers.length) {
        right += "<h4>Layers</h4><ul class=\"pb-layers\">" + r.layers.map(function (l, i) {
          var note = a.notes && a.notes[r.layer_ids[i]];
          return '<li><span class="pb-dot ' + STATE_CLASS[r.states[i]] + '"></span><span>' + esc(l) + ' <span class="pb-faint">· ' + STATE_WORD[r.states[i]] + "</span>" + (note ? '<div class="pb-note">' + esc(note) + "</div>" : "") + "</span></li>";
        }).join("") + "</ul>";
        if (r.retired) right += '<div class="pb-note">A retired transcription, made against library commit ' + esc(String(r.retired.to_sha)) + ", read: " + esc(r.layers.map(function (l, i) { return l + ": " + r.retired.states[i]; }).join("; ")) + ". It is not the current report.</div>";
      }
    ...
    function topicHeader(t, members) {
      ...
      var scope = (state.q || state.show !== "all") ? " matching" : "";
      return '<tr class="pb-topic"><td colspan="6"><span class="pb-tname">' + esc(t) + '</span><span class="pb-tstat">' + plural(members.length, scope + " roadmap") + " · " + plural(n, "layer") + ": <b>" + c.done + "</b> reported done, <b>" + c.partial + "</b> partial, " + c.untouched + " untouched, " + c.unassessed + " unassessed · <b>" + last30 + "</b> PRs merged in 30 days</span></td></tr>";
    }

    // ---- ordering ----
    function doneFrac(r) { var ag = aggregate(r), c = ag ? ag.c : counts(r.states), n = ag ? ag.n : r.layers.length; return n ? c.done / n : -1; }
    ...
    var sorters = {
      ...
      done: function (a, b) { return (doneFrac(b) - doneFrac(a)) || a.name.localeCompare(b.name); },
    ...
    function renderRows() {
      ...
      function emit(r) {
        var ch = kids(r.name);
        var chVisible = ch.filter(visible);
        var self = visible(r);
        if (!self && !chVisible.length) return;
        out.push(rowHtml(r, !self)); shown += self ? 1 : 0;
        var expand = state.kids.indexOf(r.name) >= 0 || (state.q && chVisible.length);
        if (expand) chVisible.sort(state.sort === "name" ? sorters.name : s).forEach(function (c) { out.push(rowHtml(c, false)); shown++; });
      }
      if (state.group === "topic") {
        data.topics.concat(["Unsorted"]).forEach(function (t) {
          ...
          out.push(topicHeader(t, members.filter(visible)));
    ...
    // ---- assemble ----
    var head = data.roadmap_head ? "TauCetiRoadmap@" + esc(data.roadmap_head.slice(0, 7)) : "TauCetiRoadmap";
    var stale = daysBetween(generated, new Date().toISOString());
    root.innerHTML =
      legend() +
      '<p class="pb-lede">Data collected ' + esc(generated.replace("T", " ").replace("Z", " UTC")) + " from <code>" + head + "</code> and the pull requests merged by then" +
        (stale > 2 ? ', <b>' + plural(stale, "day") + " ago</b>; dates below are relative to that collection, not to today" : "; dates below are relative to that collection") + ".</p>" +
      tiles() +
      '<div class="pb-controls" aria-label="Group, sort and filter">' +
    ...
    root.addEventListener("click", function (e) {
      ...
      if (b.hasAttribute("data-open")) {
        var name = b.getAttribute("data-open"), i = state.open.indexOf(name);
        if (i >= 0) state.open.splice(i, 1); else state.open.push(name);
        writeUrl(false); renderRows();
        var again = root.querySelector('[data-open="' + name.replace(/"/g, '\\"') + '"]');
        ...
        writeUrl(false); renderRows();
        var again2 = root.querySelector('[data-kids="' + p.replace(/"/g, '\\"') + '"]');
        if (again2) again2.focus();
      } else if (b.classList.contains("pb-reset")) {
        state.q = ""; state.show = "all";
        writeUrl(true); renderRows();
        root.querySelector(".pb-search").focus();
      } else if (b.hasAttribute("data-copy")) {
        var n = b.getAttribute("data-copy");
        var q = new URLSearchParams({ open: n });
        var parent = byName[n] && byName[n].parent;
        if (parent) q.set("kids", parent);
        var url = location.origin + location.pathname + "?" + q.toString() + "#rm-" + n;
